...
y_test <- test[, 1]
# buiding model
dp_model <- h2o.deeplearning(x = 2:785, y = 1, training_frame = train, hidden = c(1024, 1024), loss = c("CrossEntropy"), 
                             epochs = 200,  rate=0.01,  activation = "RectifierWithDropout", rate_decay = 1.0, momentum_start = 0.5,
...
write_csv(pred_data, path = "result/result.csv")
